...
from backend.orchestration.direct_research_orchestrator import (
    DirectResearchOrchestrator,
)
from backend.orchestration.direct_production_orchestrator import (
    DirectProductionOrchestrator,
)
...
async def run_pipeline(topic: str, output_format: str = "json"):
    ...
    logger = create_app_logger(
        log_dir=settings.paths.logs_dir, level=settings.log_level
    )
    ...
    try:
        ...
        research_output, _ = await research_orch.execute_research(
            topic, save_output=False
        )

        logger.info(
            f"✅ Research complete: {len(research_output)} chars (Crisp Edition)"
        )
        ...
        research_path = output_mgr.save_research_report(
            ...
            metadata={
                "orchestrator": "DirectResearchOrchestrator",
                "edition": "Crisp-Production",
            },
        )
        ...
        production_json, session_id = await production_orch.execute_production(
            research_output, save_output=True, output_format=output_format
        )
        ...
        if output_format == "json":
            ...
            logger.info(
                f"   Bengali Dialogue: {len(production_json['scenes'][0]['dialogue_bengali'])} chars"
            )

            # Save for final delivery
            delivery_path = output_mgr.save_session_data(
                f"delivery_{session_id}", production_json
            )
        ...
        if output_format == "json":
            ...
            logger.info(
                f"Delivery Path: {delivery_mgr_path if 'delivery_mgr_path' in locals() else 'Check outputs/sessions'}"
            )
