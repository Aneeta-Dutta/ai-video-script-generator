...
class ExecutionReporter:
    ...
    def generate_session_report(
        ...
        for agent_exec in session.agent_executions:
            report["agent_executions"].append({
                ...
                "completed_at": agent_exec.completed_at.isoformat() if agent_exec.completed_at else None,
                "status": agent_exec.status,
                "duration_ms": agent_exec.duration_ms,
                "output_preview": agent_exec.output[:200] if agent_exec.output else None,
        ...
        # Metrics
        if include_metrics:
            report["performance_metrics"] = self._get_session_metrics(session.session_id)
        
        # Errors
        if include_errors:
            session_errors = self.error_tracker.get_errors_by_session(session.session_id)
            report["errors"] = {
    ...
    def generate_summary_report(
        ...
        return {
            ...
            "total_errors": sum(len(self.error_tracker.get_errors_by_session(s.session_id)) for s in sessions)
        }
