...
class OutputManager:
    ...
    def __init__(self, use_cloud: bool = False):
        ...
        self.use_cloud = use_cloud or os.getenv('USE_CLOUD_STORAGE', 'false').lower() == 'true'
        ...
        if self.use_cloud:
            try:
                ...
                self.gcs_client = storage.Client(project=self.settings.api.google_cloud_project)
                bucket_name = os.getenv('GCS_BUCKET_NAME', f"{self.settings.api.google_cloud_project}-outputs")
    ...
    def save_research_report(self, topic: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def _add_metadata_header(self, content: str, metadata: Optional[Dict[str, Any]], doc_type: str) -> str:
        ...
        if self.use_cloud:
            ...
            if output_type == 'all':
                for dir_path in dirs.values():
                    if dir_path.exists():
                        outputs.extend([str(f.absolute()) for f in dir_path.iterdir() if f.is_file()])
            else:
                dir_path = dirs.get(output_type)
                if dir_path and dir_path.exists():
                    outputs = [str(f.absolute()) for f in dir_path.iterdir() if f.is_file()]
