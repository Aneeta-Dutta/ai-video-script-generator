...
class DirectProductionOrchestrator:
    ...
    async def execute_production(
        ...
        self.logger.info(f"🚀 Starting [PROD-HOUSE] Sequential Production: {scene_concept[:100]}...", session_id=session_id)
        ...
        try:
            ...
            self.logger.info("🎬 Planning cinematic sequence (Executive Producer)...", session_id=session_id)
            from backend.agents.production_agents.executive_producer import executive_producer_agent
            from google.genai import Client
            
            client = Client(
                vertexai=self.settings.api.use_vertex_ai,
                project=self.settings.api.gcp_project if self.settings.api.use_vertex_ai else None,
                location=self.settings.api.gcp_location if self.settings.api.use_vertex_ai else None,
                api_key=None if self.settings.api.use_vertex_ai else self.settings.api.google_api_key
            )
            
            plan_prompt = f"Topic & Research: {scene_concept}\n\nTask: {executive_producer_agent.instruction}"
            plan_response = client.models.generate_content(
                model=executive_producer_agent.model or self.settings.model.default_model,
            ...
            if "### 1. CREATIVE BRIEF" in ep_output:
                creative_brief = ep_output.split("### 1. CREATIVE BRIEF")[1].split("### 2. BEAT SHEET")[0].strip()
            ...
            beats = [b.strip() for b in beat_sheet.split('\n') if b.strip() and (b.strip()[0].isdigit() or b.strip().startswith('-'))]
            beats = beats[:10]  # Max 10 scenes
            
            self.logger.info(f"✅ Planned {len(beats)} beats for the short movie.", session_id=session_id)
            ...
            try:
                ...
                import re
                json_blocks = re.findall(r'```json\s*(\{.*?\})\s*```', ep_output, re.DOTALL)
                for block in json_blocks:
                    try:
                        p_data = json.loads(block)
                        if "name" in p_data and "role" in p_data:
                            self.persona_manager.save_persona(p_data["name"], p_data["role"], p_data)
            ...
            persona_context = f"\nCHARACTER PERSONAS:\n{self.persona_manager.format_persona_context(personas_found)}" if personas_found else ""
            ...
            for i, beat in enumerate(beats):
                ...
                # Extract beat type
                beat_type = "BUILD"
                if "[INTRO]" in beat.upper(): beat_type = "INTRO"
                elif "[CLIMAX]" in beat.upper(): beat_type = "CLIMAX"
                elif "[OUTRO]" in beat.upper(): beat_type = "OUTRO"
                
                self.logger.info(f"🎥 Executing Scene {scene_num}/{len(beats)} [{beat_type}]: {beat[:50]}...", session_id=session_id)
                ...
                for agent_name, agent, description in self.sub_agents:
                    ...
                    char_state_context = f"\nCHARACTER STATES: {character_states}" if character_states else ""
...
Deliver your output for this 8-SECOND BYTE. Be extremely brief."""
                    ...
                    if agent_name == "director" and "CHARACTER STATE UPDATE:" in agent_output:
                        try:
                            state_line = [l for l in agent_output.split('\n') if "CHARACTER STATE UPDATE:" in l][0]
                            state = state_line.split("CHARACTER STATE UPDATE:")[1].strip()
                ...
                scene_json = self._synthesize_scene_json(beat, sub_agent_results, scene_num, len(beats), beat_type=beat_type)
    ...
    def _synthesize_scene_json(self, beat: str, results: dict, scene_num: int, total_scenes: int, beat_type: str = "BUILD") -> dict:
        ...
        try:
            ...
            writer_data = {
                ...
                "spoken_dialogue": {
                    "text_bengali": extract_crude("text_bengali", writer_text) or extract_crude("dialogue_bengali", writer_text),
                    "text_english_ref": extract_crude("text_english_ref", writer_text) or extract_crude("dialogue_english_ref", writer_text),
                ...
                "voice_dna_applied": extract_crude("voice_dna_applied", writer_text) or "N/A"
            }

        veo3_stack = self._generate_veo_prompt(beat, results, voice_dna=writer_data.get("voice_dna_applied", ""))
        ...
        return {
            ...
            "dialogue_bengali": writer_data.get("spoken_dialogue", {}).get("text_bengali", ""),
            "dialogue_english_ref": writer_data.get("spoken_dialogue", {}).get("text_english_ref", "")
        }
    ...
    def _synthesize_script(self, concept: str, results: dict) -> str:
        ...
        if results.get("director"):
            script_parts.append("### 🎥 Layer 1: Subject Anchor & Action Vector (The Nolan Protocol)\n\n")
            script_parts.append(results["director"])
            script_parts.append("\n\n---\n\n")
        
        # Production Design (Environment & Materiality)
        if results.get("designer"):
            script_parts.append("### 🏗️ Layer 2: The Environment Mesh (Bengal Palette)\n\n")
            script_parts.append(results["designer"])
            script_parts.append("\n\n---\n\n")
        
        # Cinematography (Lighting & Camera Physics)
        if results.get("dop"):
            script_parts.append("### 📹 Layer 3: The Lighting Engine & Camera Physics\n\n")
            script_parts.append(results["dop"])
            script_parts.append("\n\n---\n\n")
        
        # Color Grading (Post-Process Stack)
        if results.get("colorist"):
            script_parts.append("### 🎨 Layer 4: The Post-Process Stack (Film Emulation)\n\n")
    ...
    def _generate_veo_prompt(self, concept: str, results: dict, voice_dna: str = "") -> str:
        ...
        def clean_line(line):
            ...
            prefixes = [
                "Stack Anchor:", "Technical Anchor:", "Anchor:", "First Line:", "Draft:", "Script:",
                "Visual Anchor:", "Subject:", "Subject Anchor:", "Scene Anchor:", "Character State Update:",
                "Director Note:", "Director's Note:"
            ]
            line = line.strip('# *')
            for p in prefixes:
