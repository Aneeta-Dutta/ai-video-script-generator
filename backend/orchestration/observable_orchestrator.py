...
from backend.core.logging import get_metrics, get_error_tracker, ErrorCategory, ErrorSeverity
...
class ObservableOrchestrator:
    ...
    @asynccontextmanager
    async def track_execution(self, operation_name: str, session_id: Optional[str] = None):
        """
        ...
        try:
            ...
            duration = self.metrics.stop_timer(timer_name, labels={"operation": operation_name})
            self.logger.info(
            ...
            duration = self.metrics.stop_timer(timer_name, labels={"operation": operation_name, "error": str(e)})
