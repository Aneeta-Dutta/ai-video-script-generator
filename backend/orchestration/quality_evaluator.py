...
class QualityEvaluator:
    ...
    def evaluate_research_report(self, report: str) -> QualityReport:
        ...
        has_data = any(
            ...
            for keyword in ["data", "statistics", "percent", "%", "number"]
        )
        checks.append(
            QualityCheck(
                name="Data Presence",
                ...
                message=(
                    "Research contains data points"
                    if has_data
                    else "Missing concrete data"
                ),
        ...
        has_narrative = any(
            keyword in report.lower()
            for keyword in ["story", "narrative", "scene", "concept"]
        )
        checks.append(
            QualityCheck(
                ...
                message=(
                    ...
                    if has_narrative
                    else "Limited narrative structure"
                ),
    ...
    def evaluate_production_script(self, script: str) -> QualityReport:
        ...
        has_veo_elements = any(
            keyword in script for keyword in ["SUBJECT", "CAMERA", "LIGHTING", "style"]
        )
        checks.append(
            QualityCheck(
                ...
                message=(
                    "Contains VEO prompt elements"
                    if has_veo_elements
                    else "Missing VEO prompt structure"
                ),
        ...
        has_production_details = any(
            keyword in script.lower()
            for keyword in ["camera", "lighting", "shot", "frame"]
        )
        checks.append(
            QualityCheck(
                ...
                message=(
                    ...
                    if has_production_details
                    else "Missing technical specifications"
                ),
    ...
    def _compile_report(self, checks: List[QualityCheck]) -> QualityReport:
        ...
        overall_score = (
            sum(check.score for check in checks) / len(checks) if checks else 0.0
        )
